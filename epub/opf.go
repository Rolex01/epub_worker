...
func (e Epub) GetStyle() (styles [][]byte) {
	m := e.GetManifest()
	for i, val := range m {
		if strings.Contains(val.Href, ".css") {
			fmt.Println("Manifest:", i, val.ID, val.Href)
			f, _ := e.OpenFile(val.Href)
			buf := new(bytes.Buffer)
			buf.ReadFrom(f)
			styles = append(styles, buf.Bytes())
		}
	}
...
func (e Epub) GetManifest() []manifest {
